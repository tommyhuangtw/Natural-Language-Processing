...
lda = LDA()
allVectors = np.array([])
util = utility('../code_toxic_detector/pos_neg_matrix_toxic.pkl','../code_toxic_detector/model_toxic.pkl', '../code_toxic_detector/vectorizer_toxic.pkl')
k = 50
...
def index(request):
    return render(request, 'classifier2/index.html')

@csrf_exempt
def predict(request):
    global allVectors, util, lda
    
    data = util.getSimilarityInfo([request.POST['sentence']], isToxic = True)
...
def initialize(request):
    ...
    data2 = util.getSimilarityInfo([request.POST['sentence']], isToxic = True)
    ...
    # }
    return render(request, 'classifier2/result.html', data)