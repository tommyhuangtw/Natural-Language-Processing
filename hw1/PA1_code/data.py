...
def learn_trigram(data ,thres = 4, verbose=True):
    ...
    if verbose:
        ...
        print("sample 1: ", " ".join(str(x) for x in sampler.sample_sentence(['With'])))
        print("sample 2: ", " ".join(str(x) for x in sampler.sample_sentence(['Next'])))
    return trigram

########__________learn_Bigram____________####################
    
def learn_bigram(data, verbose=True):
    """Learns a unigram model from data.train.
    ...
    if verbose:
        ...
        from generator import Sampler
        sampler = Sampler( bigram , temp= 0.25)
        print("sample 1: ", " ".join(str(x) for x in sampler.sample_sentence(['With'])))
        print("sample 2: ", " ".join(str(x) for x in sampler.sample_sentence(['Next'])))
    return bigram
...
if __name__ == "__main__":
    ...
    #dnames = ["brown" , "reuters"]
    datas = []
    models = []
    
    # Learn the models for each of the domains, and evaluate it
    for ngram in [ learn_trigram]:
        for dname in dnames:
            print("-----------------------")
            print(dname)
            data = read_texts("data/corpora.tar.gz", dname)
            datas.append(data)
            print('USING ',str(ngram))
            model = ngram(data, thres = 10)
            #model = learn_bigram(data)
            #print(data.train)
            #model.print_corps(data)
            models.append(model)
        ...
        vdel=[(i*0.1)+0.1 for i in range(9)]
        ...
        print("-------------------------------")
        print("x train")
        print_table(perp_train, dnames, dnames, str(ngram)+"table-train.tex")
        print("-------------------------------")
        print("x dev")
        print_table(perp_dev, dnames, dnames, str(ngram)+"table-dev.tex")
        print("-------------------------------")
        print("x test")
        print_table(perp_test, dnames, dnames, str(ngram)+"table-test.tex")
