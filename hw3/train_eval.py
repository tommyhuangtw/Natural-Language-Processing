...
if __name__ == "__main__":
    ...
    count_file_name='gene_DiffRare.counts'
    output_file_name = 'gene_train_DiffRare.p1.out'

    ########### Read gene.counts and write prediction into 'gene_dev.p1.out' #########

    print('Reading gene.counts')
    gene_count = open(count_file_name,"r")
    counter.read_counts(gene_count)
    gene_count.close()
    #counter1.read_counts(gene_count)
    #print(counter1.emission_counts)

    gene_count = open(count_file_name,"r")
    dev_file = open('train.key',"r+")

    output_file = open( output_file_name ,"w")
    counter.tagger(gene_count, dev_file, output_file)
    output_file.close()
    gene_count.close()
    dev_file.close()
